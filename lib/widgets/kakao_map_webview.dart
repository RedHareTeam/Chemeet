...
<head>
  ...
  <style>
    * { margin: 0; padding: 0; box-sizing: border-box; font-family: "Noto Sans KR", -apple-system, "Apple SD Gothic Neo", system-ui, sans-serif; -webkit-user-select: none; -webkit-touch-callout: none; user-select: none; }
    ...
    .direction-pin .pin-msg { position: absolute; bottom: 54px; left: 50%; transform: translateX(-50%); background: rgba(0,0,0,0.75); color: white; padding: 4px 8px; border-radius: 8px; font-size: 11px; white-space: nowrap; max-width: 130px; overflow: hidden; text-overflow: ellipsis; pointer-events: none; }
  </style>
</head>
<body>
  <div id="map"></div>
  ...
  <script>
    ...
    function renderBubbles(cd,sp){var wrap=document.createElement('div');wrap.className='bubble-wrap';wrap.style.left=sp.x+'px';wrap.style.top=(sp.y-20)+'px';var nt=document.createElement('div');nt.className='name-tag';nt.innerText=cd.userName;wrap.appendChild(nt);var msgs=cd.messages.slice(-4);msgs.forEach(function(msg,i){var op=msgs.length===1?1.0:0.25+(i/(msgs.length-1))*0.75;var b=document.createElement('div');b.className='bubble';b.style.background=hexToRgba(cd.color,0.85);b.style.opacity=op;b.innerText=msg;wrap.insertBefore(b,nt);});overlayContainer.appendChild(wrap);}
